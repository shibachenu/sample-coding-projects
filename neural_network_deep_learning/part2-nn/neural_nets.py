...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        vec_outout_deriative = np.vectorize(output_layer_activation_derivative)
        output_deriv = vec_outout_deriative(activated_output)

        output_layer_error = (activated_output - y) * output_deriv

        vec_hidden_deriative  = np.vectorize(rectified_linear_unit_derivative)
        hidden_deriv = vec_hidden_deriative(hidden_layer_activation)

        weighted_output_error = np.matmul(output_layer_error, self.hidden_to_output_weights)
        hidden_layer_error = np.multiply(weighted_output_error.T, hidden_deriv) # (3 by 1 matrix)
