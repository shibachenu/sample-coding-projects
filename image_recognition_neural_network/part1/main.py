...
#for lambda_factor in [1, 0.1, 0.01]:
 #   print('Linear Regression test_error =', run_linear_regression_on_MNIST(lambda_factor), ", for lambda: "+str(lambda_factor))
...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    print("Test error before mod3: "+str(test_error))
    # Save the model parameters theta obtained from calling softmax_regression to disk.
    write_pickle_data(theta, "./theta.pkl.gz")

    # Read theta from disk
    train_x, train_y, test_x, test_y = get_MNIST_data()
    theta = read_pickle_data("./theta.pkl.gz")
    test_error = compute_test_error_mod3(test_x, test_y % 3, theta, temp_parameter)
    print("Test error with mod3: "+str(test_error))
    return test_error

# TODO: Find the error rate for temp_parameter = [.5, 1.0, 2.0]
#      Remember to return the tempParameter to 1, and re-run run_softmax_on_MNIST
#for temp_parameter in [0.5, 1, 2]:
 #   print('softmax test_error=', run_softmax_on_MNIST(temp_parameter), "for temp: "+str(temp_parameter))
...
def run_softmax_on_MNIST_mod3(temp_parameter=1):
    ...
    # train with mod data
    theta, cost_function_history = softmax_regression(train_x, train_y % 3, temp_parameter, alpha=0.3,
                                                      lambda_factor=1.0e-4,
                                                      k=10, num_iterations=150)
    test_error = compute_test_error(test_x, test_y % 3, theta, temp_parameter)
    return test_error

#print('softmax test_error for updated mod model=', run_softmax_on_MNIST_mod3(temp_parameter=1))
...
#
# print("test error using PCA data: " + str(test_error))
...
print("test error using PCA cubed: " + str(test_error))