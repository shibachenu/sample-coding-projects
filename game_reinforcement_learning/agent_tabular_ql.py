...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    import math
    # current Q
    q_pre = q_func[current_state_1, current_state_2, action_index, object_index]

    q_nextstate_max = 0 if terminal else np.max(q_func[next_state_1, next_state_2, :, :])
    # updated Q
    q_post = (1 - ALPHA) * q_pre + ALPHA * (reward + GAMMA * q_nextstate_max)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        current_state = dict_room_desc[current_room_desc]
        current_quest = dict_quest_desc[current_quest_desc]
        (action_index, object_index) = epsilon_greedy(current_state, current_quest, q_func, epsilon)
        ...
        if for_training:
            # update Q-function.
            tabular_q_learning(q_func, current_state, current_quest, action_index, object_index, reward,
