...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    """
    is_random = np.random.binomial(n=1, p=epsilon)
    if is_random:
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    q_pre = (theta @ current_state_vector)[action_object_index]
    q_next = 0 if terminal else np.max(theta @ next_state_vector)

    gradient = (reward + GAMMA * q_next - q_pre) * current_state_vector

    # updated theta
    theta[action_object_index] = theta[action_object_index] + ALPHA * gradient
# pragma: coderesponse end
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        current_room_bow = utils.extract_bow_feature_vector(current_room_desc + current_quest_desc, dictionary)
        (action_index, object_index) = epsilon_greedy(current_room_bow, theta, epsilon)
        (next_room_desc, next_quest_desc, reward, terminal) = framework.step_game(current_room_desc, current_quest_desc,
                                                                                  action_index, object_index)
        next_room_bow = utils.extract_bow_feature_vector(next_room_desc + next_quest_desc, dictionary)

        if for_training:
            # update Q-function.
            linear_q_learning(theta, current_room_bow.T, action_index, object_index,
                      reward, next_room_bow.T, terminal)
            pass
