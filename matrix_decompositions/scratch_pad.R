...
angle = sum(a * b)/(norm(a)*norm(b))


projection_scalar = sum(a * b)/norm(b)

qnorm(1-0.015)

pnorm(1/sqrt(2))-pnorm(-0.5/sqrt(2))

pnorm(2, 1, sqrt(2)) - pnorm(0.5, 1, sqrt(2))
...
(sum((Xs-mean(Xs))*(Ys-mean(Ys)))/(sd(Xs)*sd(Ys)))/(N-1)

beta1 = (cor(Xs, Ys)*sd(Ys))/sd(Xs)

reg = lm(Ys~Xs)
summary(reg)

beta0 = mean(Ys) - beta1 * mean(Xs)
...
#Y is LogPlanetMass
#X is intercept, LogPlanetRadius, LogPlanetOrbit, StarMetallicity, LogStarMass, LogStarAge

X_dummy = rep(1, N)
X = cbind(X_dummy, LogPlanetRadius, LogPlanetOrbit, StarMetallicity, LogStarMass, LogStarAge)
